...
function _makeOutsiderAssignments(devTeams, devByName, prTeamSize) {
  ...
  for (var devTeamIndex = 0; devTeamIndex < devTeams.length; devTeamIndex++) {
    ...
    for (var devNameIndex = 0; devNameIndex < devNames.length; devNameIndex++) {
      ...
      assignReviewers:
      for (var nextDevTeamCount = 0; nextDevTeamCount < devTeams.length; nextDevTeamCount++) {
...
// - PR teams will consist of exactly <PR team size> devs
// - Each dev will be be a reviewer for exactly (<PR team size> - 1) authors
// - PR teams will consist of all dev teammates, limited by the "cross-pollination" constraint
// - At least 1 dev from outside a dev team will be on each PR team for "cross-pollination",
...
//   likely that dev team 2 will audit dev team 1 than a smattering of dev teams 2 - 3 will audit
//   dev team 1
function generatePrTeams(devTeams, prTeamSize, minOutsiders) {
  var devByName;
  ...
  if (assignmentsAttempts === 10) {
    throw new Error('Infinite loop condition.');
  }

  return Object.keys(devByName).map(function (devName) {
    return devByName[devName];
  }).map(devToString).join('\n');
}
